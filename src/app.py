            ...
            for episode_data in new_episodes:
                try:
                    ...
                    if download_with_yt_dlp(**download_params):
                        ...
                        original_series_index = next((idx for idx, s in enumerate(current_config['series']) if s['name'] == series['name']), None)
                        if original_series_index is not None:
                            current_config['series'][original_series_index]['last'] = episode_data['episode']
